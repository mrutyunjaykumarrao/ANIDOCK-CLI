...
PyInstaller builds are OS-specific. Build on each target OS to distribute a runnable binary.
...
The executable will be at `dist/anidock`. Ensure `ffmpeg` is installed and on PATH.

## Automated Releases (GitHub Actions)

Builds are OS-specific, so use GitHub Actions to produce Windows, macOS, and Linux binaries on tag pushes.
...
Artifacts will appear in the workflow run as:

- `anidock-windows-latest` (contains `anidock.exe`)
- `anidock-macos-latest`
- `anidock-ubuntu-latest`
...
```
On Windows, the manifest is visible in the download folder.
...
On Windows, install via winget or Chocolatey and ensure `ffmpeg` is on PATH:
